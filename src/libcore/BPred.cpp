  ...
  I(bpred4CycleAddrShift>=0 && 
   ...
   if (strstr(name,"RAS")==0 && strstr(name,"BTB")==0)
   {
      ...
      for(size_t counter = 0; counter < NUM_STATES; counter++)
      {
         ...
         bpredEnergy[counter] = new GStatsEnergy(fullEntry.c_str(), cadena, i, FetchPower, EnergyMgr::get(fullComponent.c_str(), i));
      }
#else
...
BPred::~BPred()
{
   ...
   for(size_t counter = 0; counter < NUM_STATES; counter++)
   {
      if(bpredEnergy[counter])
         delete bpredEnergy[counter];
   }
#else
   if(bpredEnergy)
      delete bpredEnergy;
   ...
   ,RasSize(SescConf->getInt(section,"rasSize"))
{
  ...
  for(size_t counter = 0; counter < NUM_STATES; counter++)
  {
     fullEntry = entry + GProcessor::state_to_freq(counter);
     fullComponent = component + GProcessor::state_to_freq(counter);
     rasEnergy[counter] = new GStatsEnergy(fullEntry.c_str(), cadena, i, FetchPower, EnergyMgr::get(fullComponent.c_str(), i));
  }
#else
...
PredType BPRas::predict(const Instruction *inst,
#if defined(SEP_DVFS)
  ...
  :BPred(i, fetchWidth, section, name ? name : "BTB")
{
  ...
  for(size_t counter = 0; counter < NUM_STATES; counter++)
  {
     fullEntry = entry + GProcessor::state_to_freq(counter);
     fullComponent = component + GProcessor::state_to_freq(counter);
     btbEnergy[counter] = new GStatsEnergy(fullEntry.c_str(), cadena, i, FetchPower, EnergyMgr::get(fullComponent.c_str(),i));
  }
#else
...
PredType BPBTB::predict(const Instruction * inst,
#if defined(SEP_DVFS)
                        unsigned int executeState,
#endif
                        InstID oracleID, bool doUpdate)
...
PredType BPOracle::predict(const Instruction * inst,
#if defined(SEP_DVFS)
                           unsigned int executeState,
#endif
                           InstID oracleID, bool doUpdate)
...
#if defined(SEP_DVFS)
  return btb.predict(inst, executeState, oracleID, doUpdate);
#else
...
PredType BPTaken::predict(const Instruction * inst,
#if defined(SEP_DVFS)
                          unsigned int executeState,
#endif
                          InstID oracleID, bool doUpdate)
...
PredType  BPNotTaken::predict(const Instruction * inst,
#if defined(SEP_DVFS)
                              unsigned int executeState,
#endif
                              InstID oracleID, bool doUpdate)
...
PredType BPStatic::predict(const Instruction * inst,
#if defined(SEP_DVFS)
                           unsigned int executeState,
#endif
                           InstID oracleID, bool doUpdate)
...
#if defined(SEP_DVFS)
  return ptaken ? btb.predict(inst, executeState, oracleID, doUpdate) : CorrectPrediction;
#else
...
PredType BP2bit::predict(const Instruction *inst,
#if defined(SEP_DVFS)
                         unsigned int executeState,
#endif
                         InstID oracleID, bool doUpdate)
...
PredType BP2level::predict(const Instruction * inst,
#if defined(SEP_DVFS)
                           unsigned int executeState,
#endif
                           InstID oracleID, bool doUpdate)
...
PredType BPHybrid::predict(const Instruction *inst,
#if defined(SEP_DVFS)
                           unsigned int executeState,
#endif
                           InstID oracleID, bool doUpdate)
...
PredType BP2BcgSkew::predict(const Instruction * inst,
#if defined(SEP_DVFS)
                             unsigned int executeState,
#endif
                             InstID oracleID, bool doUpdate)
...
PredType BPyags::predict(const Instruction *inst,
#if defined(SEP_DVFS)
                         unsigned int executeState,
#endif
                         InstID oracleID, bool doUpdate)
...
PredType BPOgehl::predict(const Instruction *inst,
#if defined(SEP_DVFS)
                          unsigned int executeState,
#endif
                          InstID oracleID, bool doUpdate)
