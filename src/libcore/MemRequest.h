...
protected:
  ...
  // Called through callback
   ...
   unsigned int fakeAbort(unsigned int processorID, unsigned int versioning) { return 0; /*warnigns*/ };
   unsigned int fakeCommit(unsigned int processorID, unsigned int versioning) { return 0; /*warnigns*/ };

   int   procExecuteState;
public:
   int get_executeState(void) { return procExecuteState; }
   void set_executeState(unsigned int newState) { procExecuteState = newState; }
...
class IMemRequest : public MemRequest {
 ...
 public:
#ifdef SESC_ENERGY
   static void create(DInst *dinst, GMemorySystem *gmem, IBucket *buffer, unsigned int executeState);
#else
