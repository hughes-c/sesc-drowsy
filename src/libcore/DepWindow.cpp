  ...
  ,nReplay("Proc(%d)_%s:nReplay", gp->getId(), clusterName)
{
  ...
  entry = "windowSelEnergy";
  component = "windowSelEnergy";
  for(size_t counter = 0; counter < NUM_STATES; counter++)
  {
     ...
     windowSelEnergy[counter] = new GStatsEnergy(fullEntry.c_str(), cadena , Id, IssuePower
            ,EnergyMgr::get(fullComponent.c_str(), Id));
  }

  entry = "windowRdWrEnergy";
  component = "windowRdWrEnergy";
  for(size_t counter = 0; counter < NUM_STATES; counter++)
  {
     fullEntry = entry + GProcessor::state_to_freq(counter);
     fullComponent = component + GProcessor::state_to_freq(counter);
     windowRdWrEnergy[counter] = new GStatsEnergy(fullEntry.c_str(), cadena , Id, IssuePower
            ,EnergyMgr::get(fullComponent.c_str(), Id));
  }

  entry = "windowCheckEnergy";
  component = "windowCheckEnergy";
  for(size_t counter = 0; counter < NUM_STATES; counter++)
  {
     fullEntry = entry + GProcessor::state_to_freq(counter);
     fullComponent = component + GProcessor::state_to_freq(counter);
     windowCheckEnergy[counter] = new GStatsEnergy(fullEntry.c_str(), cadena , Id, IssuePower
            ,EnergyMgr::get(fullComponent.c_str(), Id));
  }
#else
...
#if defined(SEP_DVFS)
  unsigned int procExecState = gproc->get_executeState();
  windowCheckEnergy[procExecState]->inc();
  windowSelEnergy[procExecState]->inc();
  windowRdWrEnergy[procExecState]->inc();  // Add entry
  windowRdWrEnergy[procExecState]->inc();  // check deps
  windowRdWrEnergy[procExecState]->inc();  // Remove the entry
#else
