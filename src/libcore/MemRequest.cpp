...
void DMemRequest::create(DInst *dinst, GMemorySystem *gmem, MemOperation mop)
{
  ...
  if(!context->isValidDataVAddr(old_addr)){
   ...
   #ifdef SESC_ENERGY
   r->set_executeState(dinst->procExecuteState);
   #endif
...
#ifdef SESC_ENERGY
void IMemRequest::create(DInst *dinst, GMemorySystem *gmem, IBucket *bb, unsigned int executeState)
#else
...
#endif
{
   ...
   #ifdef SESC_ENERGY
   r->set_executeState(executeState);
   #endif
