...
protected:
  ...
  // Energy Counters
#if defined(SEP_DVFS)
  GStatsEnergyBase *renameEnergy[NUM_STATES];
  GStatsEnergyBase *robEnergy[NUM_STATES];

  GStatsEnergyBase *wrRegEnergy[3][NUM_STATES]; // 0 INT, 1 FP, 2 NONE  -- [reg][dvfs]
  GStatsEnergyBase *rdRegEnergy[3][NUM_STATES]; // 0 INT, 1 FP, 2 NONE  -- [reg][dvfs]
#else
...
#ifdef SESC_ENERGY
   public:
      size_t get_executeState(void) { return executeState; }
      void set_executeState(size_t newState) { executeState = newState; }

      size_t get_skipAmount(void) { return skipAmount; }
      void set_skipAmount(size_t skipped) { skipAmount = skipped; }

      static std::string state_to_freq(size_t state);
      static double state_to_freq_dbl(size_t state);
      static size_t freq_to_state(std::string freq);

      static double totalExecTime(void);
      static double procGateTime(size_t procID);
...
#if (defined TM)
  public:
     void incrementSignatureEnergy(void) { signatureEnergy->inc(); }

  protected:
     GStatsEnergyBase *signatureEnergy;
#endif
