...
#if defined(SEP_DVFS)
  PredType prediction     = bpred->predict(inst, this->gproc->get_executeState(), oracleID, false);
#else
...
#else
#if defined(SEP_DVFS)
  PredType prediction     = bpred->predict(inst, this->gproc->get_executeState(), oracleID, !dinst->isFake());
#else
...
#if defined(SEP_DVFS)
    dinst->procExecuteState = getGProcessor()->get_executeState();
#endif
...
#ifdef SESC_ENERGY
             , getGProcessor()->get_executeState()
#endif
                         );
    else
