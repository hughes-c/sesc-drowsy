  ...
  ,memorySystem(ms)
{
  ...
  entry = "ldqCheckEnergy";
  component = "ldqCheckEnergy";
  for(size_t counter = 0; counter < NUM_STATES; counter++)
  {
     ...
     ldqCheckEnergy[counter] = new GStatsEnergy(fullComponent.c_str(), cadena, id, ExecPower
           ,EnergyMgr::get(fullComponent.c_str(), id));
  }

  entry = "ldqRdWrEnergy";
  component = "ldqRdWrEnergy";
  for(size_t counter = 0; counter < NUM_STATES; counter++)
  {
     fullEntry = entry + GProcessor::state_to_freq(counter);
     fullComponent = component + GProcessor::state_to_freq(counter);
     ldqRdWrEnergy[counter] = new GStatsEnergy(fullComponent.c_str(), cadena, id, ExecPower
           ,EnergyMgr::get(fullComponent.c_str(), id));
  }

  entry = "stqRdWrEnergy";
  component = "stqRdWrEnergy";
  for(size_t counter = 0; counter < NUM_STATES; counter++)
  {
     fullEntry = entry + GProcessor::state_to_freq(counter);
     fullComponent = component + GProcessor::state_to_freq(counter);
     stqRdWrEnergy[counter] = new GStatsEnergy(fullComponent.c_str(), cadena, id, ExecPower
           ,EnergyMgr::get(fullComponent.c_str(), id));
  }
...
#if defined(SEP_DVFS)
  stqRdWrEnergy[dinst->procExecuteState]->inc();
  ldqRdWrEnergy[dinst->procExecuteState]->inc();
#else
  stqRdWrEnergy->inc();
  ldqRdWrEnergy->inc();
#endif
...
void FUMemory::simTime(DInst *dinst)
{
  ...
  ldqCheckEnergy[dinst->procExecuteState]->inc();
  ldqRdWrEnergy[dinst->procExecuteState]->inc();
#else
  stqRdWrEnergy->inc();
...
#if (defined TM)
#ifdef SESC_ENERGY
   if(dinst->transType == transLoad)
   {
      getCluster()->getGProcessor()->incrementSignatureEnergy();
   }
#endif
#endif
