...
//
//
//Methods to return private and protected data
   char DInst::get_nDeps()
   {
      return this->nDeps;
   }

   RegType DInst::get_src1()
   {
      return inst->getSrc1();
   }
   
   RegType DInst::get_src2()
   {
      return inst->getSrc2();
   }
   
   RegType DInst::get_dest()
   {
      return inst->getDest();
   }
   
   EventType DInst::get_uEvent(void)
   {
      return inst->getEvent();
   }
   
   InstSubType DInst::get_subCode(void)
   {
      return inst->getSubCode();
   }
   
   bool DInst::get_guessTaken(void)
   {
      return inst->guessAsTaken();
   }
   
   bool DInst::get_condLikely(void)
   {
      return inst->isBJLikely();
   }
   
   bool DInst::get_jumpLabel(void)
   {
      return inst->doesJump2Label();
   }
   
   MemDataSize DInst::get_dataSize(void)
   {
      return inst->getDataSize();
   }

   bool DInst::set_immediate(int immediate)
   {
      this->immediate = immediate;
      return 1;
   }

   int  DInst::get_immediate(void)
   {
