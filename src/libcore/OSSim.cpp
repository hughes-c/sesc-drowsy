...
#ifdef SESC_THERM
std::ofstream ReportTherm::powerTraceFile;
#endif
...
// This instantiates the Global Transactional Memory reporting system as well as the Global Coherence Protocol Module
...
#ifdef SESC_THERM
  {
    ...
    *pp = 0;
    ...
    //power trace
    std::string tempProfileFile = thermFile;
    tempProfileFile = tempProfileFile.substr(0, tempProfileFile.find("therm")) + "power" + tempProfileFile.substr(tempProfileFile.find("therm") + 5);
    ReportTherm::powerTraceFile.open(tempProfileFile.c_str(), std::ios::trunc);
  }
#endif
...
#ifdef SESC_THERM
  ReportTherm::powerTraceFile.close();
#endif
...
void OSSim::report(const char *str)
{
...
#if defined(SEP_DVFS)
  totalRunTime = GProcessor::totalExecTime();
#else
  ...
  for(size_t i = 0;i < cpus.size(); i++)
  {
     GProcessor *gproc = cpus.getProcessor(i);
     if(gproc)
        totalCycles = totalCycles + gproc->getClockTicks();
  }

  totalRunTime = totalCycles / getFrequency();
#endif
  ...
  Report::field("OSSim:totalExecTime=%f", totalRunTime);
  ...
  Report::field("OSSim:pseudoreset=%lld", snapshotGlobalClock);
   ...
   {
      #if defined(SEP_DVFS)
      double pPower = GStatsEnergy::getTotalProcPower(i);
      #else
    ...
    double clockPower = GStatsEnergy::getProcClockPower(i);

    if(transGCM->useConflictProbability == 1)
       clockPower = 0.7 * clockPower;

    double corePower  = pPower + clockPower;

    std::cout << "\tpPower:  " << pPower << "   clockPower:  " << clockPower << "  corePower:  " << corePower << "\n";

    totEnergy = totEnergy + GStatsEnergy::getTotalProcEnergy(i) + EnergyMgr::ptoe(clockPower);
#else
    // More reasonable clock energy. 50% is based on activity, 50% of the clock
    // distributtion is there all the time
  ...
  #if defined(SEP_DVFS)
  Report::field("PowerMgr:totPower=%g", GStatsEnergy::getTotalPower() + totClockPower);
  Report::field("EnergyMgr:clockEnergy=%g", EnergyMgr::ptoe(totClockPower));
  Report::field("EnergyMgr:totEnergy=%g", totEnergy);
  #else
