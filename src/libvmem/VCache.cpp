  ...
  ,pushHalfMiss("%s:pushHalfMiss", name)
{
  ...
  for(size_t counter = 0; counter < NUM_STATES; counter++)
  {
     ...
     rdEnergy[0][counter] = new GStatsEnergy(fullEntry.c_str(), name, gms->getId(), MemPower
  ...
  for(size_t counter = 0; counter < NUM_STATES; counter++)
  {
     fullEntry = entry + GProcessor::state_to_freq(counter);
     fullComponent = component + GProcessor::state_to_freq(counter);
     rdEnergy[1][counter] = new GStatsEnergy(fullEntry.c_str(), name, gms->getId(), MemPower
           ,EnergyMgr::get(fullComponent.c_str(), i));
  }
  ...
  for(size_t counter = 0; counter < NUM_STATES; counter++)
  {
     fullEntry = entry + GProcessor::state_to_freq(counter);
     fullComponent = component + GProcessor::state_to_freq(counter);
     wrEnergy[0][counter] = new GStatsEnergy(fullEntry.c_str(), name, gms->getId(), MemPower
           ,EnergyMgr::get(fullComponent.c_str(), i));
  }
  ...
  for(size_t counter = 0; counter < NUM_STATES; counter++)
  {
     fullEntry = entry + GProcessor::state_to_freq(counter);
     fullComponent = component + GProcessor::state_to_freq(counter);
     wrEnergy[1][counter] = new GStatsEnergy(fullEntry.c_str(), name, gms->getId(), MemPower
           ,EnergyMgr::get(fullComponent.c_str(), i));
  }
