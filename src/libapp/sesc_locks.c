...
int timeout;
void localTimer(int time)
{
  int timer, timer2;
  for(timer = 0; timer < time; timer++)
  {
     for(timer2 = 0; timer2 < time; timer2++)
     {
     }
  }
...
int sesc_tsema(ssema_t *sema, int time)
{
  int i=0;
  timeout = 0;
  localTimer(time);
  /* DOWN, wait() */
  while(sema->count <= 0 && timeout > 0) {
    i--;
    if( i < 0 ) {
      sesc_yield(-1);
      i=1000;
...
void sesc_flag_wait_trans(sflag_t *flag, int transID)
{
  //transID is currently not defined when using the old assembly defines
  COMMIT_TRANSACTION(99);
  sesc_lock(&(flag->lock));
  if(!flag->flag){
    flag->count++;
    sesc_unlock(&(flag->lock));
  ...
  BEGIN_TRANSACTION(99);
}

void sesc_flag_wait_lock(sflag_t *flag, slock_t *depLock)
{
  sesc_unlock(depLock);
  sesc_lock(&(flag->lock));
  if(!flag->flag){
...
int sesc_flag_timedwait(sflag_t *flag, slock_t *depLock, int time)
{
  int timer = 0;
  sesc_unlock(depLock);
  sesc_lock(&(flag->lock));
  if(!flag->flag)
  {
    flag->count++;
    sesc_unlock(&(flag->lock));
    timer = sesc_tsema(&(flag->queue), time);
  }
  else
  {
    sesc_unlock(&(flag->lock));
