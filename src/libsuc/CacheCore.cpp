...
void CacheGeneric<State, Addr_t, Energy>::createStats(const char *section, const char *name)
{
  ...
  if (Energy)
  {
    ...
    for(size_t counter = 0; counter < NUM_STATES; counter++)
    {
       ...
       rdEnergy[0][counter] = new GStatsEnergy(fullEntry.c_str(), name, procId, pg
    ...
    for(size_t counter = 0; counter < NUM_STATES; counter++)
    {
       fullEntry = entry + GProcessor::state_to_freq(counter);
       fullComponent = component + GProcessor::state_to_freq(counter);
       rdEnergy[1][counter] = new GStatsEnergy(fullEntry.c_str(), name, procId, pg
             ,EnergyMgr::get(section, fullComponent.c_str()));
    }
    ...
    for(size_t counter = 0; counter < NUM_STATES; counter++)
    {
       fullEntry = entry + GProcessor::state_to_freq(counter);
       fullComponent = component + GProcessor::state_to_freq(counter);
       wrEnergy[0][counter] = new GStatsEnergy(fullEntry.c_str(), name, procId, pg
             ,EnergyMgr::get(section, fullComponent.c_str()));
    }
    ...
    for(size_t counter = 0; counter < NUM_STATES; counter++)
    {
       fullEntry = entry + GProcessor::state_to_freq(counter);
       fullComponent = component + GProcessor::state_to_freq(counter);
       wrEnergy[1][counter] = new GStatsEnergy(fullEntry.c_str(), name, procId, pg
             ,EnergyMgr::get(section, fullComponent.c_str()));
    }
  ...
  else
  {
     ...
     for(size_t counter = 0; counter < NUM_STATES; counter++)
     {
        rdEnergy[0][counter]  = 0;
        rdEnergy[1][counter]  = 0;
        wrEnergy[0][counter]  = 0;
        wrEnergy[1][counter]  = 0;
     }
#else
     rdEnergy[0]  = 0;
     rdEnergy[1]  = 0;
