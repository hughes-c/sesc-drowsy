...
#if (defined TM)
#ifdef SESC_ENERGY
   //if this is inside of a transaction and the conflict detection is eager,
   //we want to check the address against the write signature
   if(mreq->getDInst()->transType == transLoad && transGCM->getConflictDetection() == 1)
   {
      mreq->getGProcessor()->incrementSignatureEnergy();
   }
#endif
#endif
...
#ifdef SESC_ENERGY
   //if this is inside of a transaction and the conflict detection is eager,
   //we want to check the address against the read and write signatures
   if(mreq->getDInst()->transType == transLoad && transGCM->getConflictDetection() == 1)
   {
      mreq->getGProcessor()->incrementSignatureEnergy();
      mreq->getGProcessor()->incrementSignatureEnergy();
