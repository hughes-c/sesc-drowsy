...
#if defined(STAT_COMMON)
    MIPSDecodeInstruction(i, picode, opcode, tmcode, subCode, dataSize, opNum, src1, dest, src2, uEvent
			  ,BJCondLikely, gTaken, jumpLabel);
#elif (defined TM)
    MIPSDecodeInstruction(i, picode, opcode, tmcode, subCode, dataSize, src1, dest, src2, uEvent
			  ,BJCondLikely, gTaken, jumpLabel);
#else
...
#if (defined TM)
    InstTable[i].tmcode = tmcode;
#endif
    InstTable[i].dataSize= dataSize;
#if defined(STAT_COMMON)
    InstTable[i].opNum= opNum;
#endif
  ...
  )
{
  ...
  case spawn_opn:
...
#if (defined TM) // Begin User Defined TM Section
    case tmBegin_opn:
      tmcode = transBegin;
      opcode  = iALU;
      subCode = iNop;
    ...
    case tmCommit_opn:
      ...
      subCode = iNop;
      break;
#endif          // End User Defined TM Section
