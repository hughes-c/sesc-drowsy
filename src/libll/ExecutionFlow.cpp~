...
std::vector < INT_8 >   toggleOn (MAX_NUM_THREADS, 0);
std::vector < InstID >  lastInstructionTargetAddress (MAX_NUM_THREADS, 0);
#endif
...
int ExecutionFlow::exeInst(void)
{
  ...
  do{
    ...
    I(picodePC);
#if (defined TM)
    if(transGCM->checkStall(thread.getPid()))
      break;
#endif
  }while(picodePC->addr==iAddr);
...
DInst *ExecutionFlow::executePC()
{
   ...
   if(thread.get_threadID() >= lastInstructionTargetAddress.size())
   {
      if(thread.get_threadID() == lastInstructionTargetAddress.size())
      {
         std::cerr << "Synthesis::lastInstructionTargetAddress.push_back with " << thread.get_threadID();
         lastInstructionTargetAddress.push_back(0);
         std::cerr << " and new size of " << lastInstructionTargetAddress.size() << std::endl;
      }
      else
      {
         std::cerr << "Synthesis::lastInstructionTargetAddress.resize with " << thread.get_threadID();
         lastInstructionTargetAddress.resize(thread.get_threadID() + 1, 0);
         std::cerr << " and new size of " << lastInstructionTargetAddress.size() << std::endl;
      }
   }
#endif
...
#if defined(STAT_COMMON)
   dinst->resetEvents();
   PFPI tempFunc = picodePC->func;
   dinst->set_threadID(thread.get_threadID());
#endif

//Check to see if a conditional branch was taken or not taken
//by comparing the jump target of the previous instruction
//with the address of the current instruction
#if defined(STAT_COMMON)
  if(picodePC->addr == lastInstructionTargetAddress[thread.get_threadID()])
    dinst->set_isTaken(1);
  else
    dinst->set_isTaken(0);

  if(picodePC->target != NULL)
    lastInstructionTargetAddress[thread.get_threadID()] = picodePC->target->addr;
#endif
...
#if (defined TM)
  // If we are stalled, we do not want to propogate the rest of the instruction
  if(transGCM->checkStall(thread.getPid()))
    return 0;
#endif
  ...
  dinst = DInst::createInst(cPC
   ...
   //Store the current immediate value and reset the instruction events
#if defined(STAT_COMMON)
   dinst->resetEvents();
   dinst->set_immediate(origPIcode->get_immediate());
   dinst->set_instFunc(origPIcode->func);
#endif

  // If we are currently in a transaction, mark the instruction as a transactional instruction
  // The dinst->getTmcode() != transNT is used to detect explicit transactional instructions that
  // may not be within a transaction (e.g. Begin instructions)
#if (defined TM)

    // Set the default transBCFlag to -1
    dinst->transBCFlag = -1;

    if((thread.tmDepth > 0) || dinst->getTmcode() != transNT)
    {
      if(dinst->getTmcode() == transNT)
      {
        int dinstInstType = (int)dinst->getOpcode();
        switch(dinstInstType){
          case iALU:
          case iMult:
          case iDiv:
            dinst->transType = transInt;
            break;
          case iBJ:
            dinst->transType = transBJ;
            break;
          case iLoad:
            dinst->transType = transLoad;
            break;
          case iStore:
            dinst->transType = transStore;
            break;
          case fpALU:
          case fpMult:
          case fpDiv:
            dinst->transType = transFp;
            break;
          case iFence:
            dinst->transType = transFence;
            break;
          default:
            dinst->transType = transOther;
        }
      }
      else  // if it has already been assigned (e.g. Begin/Abort/Commit)
      {
        dinst->transType = dinst->getTmcode();
        dinst->transBCFlag = thread.tmBCFlag;
      }

      dinst->transPid = thread.pid;
      dinst->transTid = thread.tmTid;
    }
#endif
  ...
  if(!hasDelay) {
   ...
   //Each instance needs to be aware of its current TID
   //Identify waits here.
#if defined(STAT_COMMON)
   PFPI tempFunc = picodePC->func;
   dinst->set_threadID(thread.get_threadID());
   if(tempFunc == mint_sesc_spawn)
   {
      dinst->set_isSpawn(1);
      //We've already incremented the thread count so the next TID will be count - 1
      dinst->set_targetThread(totalNumThreads - 1);
   }
   else if(tempFunc == mint_sesc_wait)
   {
      dinst->set_isWait(1);
   }
#endif
...
//We need to store the pointer to the instruction event handler
#if (defined TM)
   PFPI tempFunc = picodePC->func;
#elif defined(STAT_COMMON)
   PFPI tempFunc = picodePC->func;
#endif
...
//NOTE New thread spawns cannot be identified beyond this point
#if defined(STAT_COMMON)
   if(tempFunc == mint_sesc_spawn)
   {
      dinst->set_isSpawn(1);
...
//with the address of the current instruction
  ...
  pendingDInst = DInst::createInst(Instruction::getEventID(ev)
   ...
   if(tempFunc == mint_sesc_fetch_op)
   {
      if(picodePC->immed == 22)
      {
         dinst->synchType = barrier;
      }
    }
  ...
  epoch->execInstr();
...
#if defined(STAT_COMMON)
   //Need to ensure that the vector is large enough to hold the next thread
   if(thread.get_threadID() >= toggleOn.size())
   {
      if(thread.get_threadID() == toggleOn.size())
      {
         std::cerr << "Synthesis::toggleOn.push_back with " << thread.get_threadID();
         toggleOn.push_back(0);
         std::cerr << " and new size of " << toggleOn.size() << std::endl;
      }
      else
      {
         std::cerr << "Synthesis::toggleOn.resize with " << thread.getPid();
         toggleOn.resize(thread.get_threadID() + 1, 0);
         std::cerr << " and new size of " << toggleOn.size() << std::endl;
      }
   }
   ...
   if(tempFunc == mint_sesc_fetch_op)
   {
      if(picodePC->immed == 22)
      {
         dinst->set_isBarrier(1);
      }
      else if(picodePC->immed == 19)
      {
            toggleOn[thread.get_threadID()] = 1;
      }
   }

   if(tempFunc == mint_sesc_memfence && toggleOn[thread.getPid()] == 1)
   {
      toggleOn[thread.get_threadID()] = 0;
      dinst->set_lockID(thread.getIntReg(IntArg1Reg));
      dinst->set_isCriticalStart(1);
   }

   if(tempFunc == mint_sesc_unlock_op)
   {
      dinst->set_isCriticalEnd(1);
   }
#endif
